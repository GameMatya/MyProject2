...
// 有効化で触れているグループのポリゴンを可視化する ( debugのみ )
//#define DRAW_POLYGON_GROUP

#endif // _DEBUG

CompStage::CompStage(const char* filepath)
{
  // リソース読み込み  ( ModelResourceで引数の3Dモデルを読み込む )
  resource = ResourceManager::Instance().LoadStaticResource(filepath);
}

void CompStage::Start()
{
  groups = resource->GetCollision().polygonGroupes;
}

void CompStage::DrawImGui()
{
  DebugRenderer* renderer = Graphics::Instance().GetDebugRenderer();

  for (int i = 0; i < groups.size(); ++i) {
    renderer->DrawSquare(groups[i].aabbCenter, groups[i].aabbRadius, { 1,0,0,1 });

    DirectX::XMFLOAT3 newRad = { groups[i].aabbRadius.x * 0.8f,groups[i].aabbRadius.y * 0.8f,groups[i].aabbRadius.z * 0.8f };
    renderer->DrawSquare(groups[i].aabbCenter, newRad, { 1,0,1,1 });
  }
}

bool CompStage::RayCast(const DirectX::XMFLOAT3& start, const DirectX::XMFLOAT3& end, HitResult* hitResult)
{
  //TODO debug
  const auto& line = Graphics::Instance().GetLineRenderer();

  DirectX::XMMATRIX InverseTransform = DirectX::XMMatrixInverse(nullptr, gameObject.lock()->transform.GetWorldTransform());
  ...
  // モデルの頂点コンテナ
  const auto& vertices = resource->GetVertices();
  // レイキャストの最終結果
  bool isHit = false;
  ...
  while (!hitGroups.empty()) {
    ...
    for (auto& vertexData : groups[groupIndex].vertexDatas) {
      // ポリゴンとレイの当たり判定
      {
        ...
        line->AddVertex(vertices[vertexData.keyID][vertexData.vertexA].position, { 1,1,1,1 });
        line->AddVertex(vertices[vertexData.keyID][vertexData.vertexA + 1].position, { 1,1,1,1 });
        line->AddVertex(vertices[vertexData.keyID][vertexData.vertexA + 1].position, { 1,1,1,1 });
        line->AddVertex(vertices[vertexData.keyID][vertexData.vertexA + 2].position, { 1,1,1,1 });
        line->AddVertex(vertices[vertexData.keyID][vertexData.vertexA + 2].position, { 1,1,1,1 });
        line->AddVertex(vertices[vertexData.keyID][vertexData.vertexA].position, { 1,1,1,1 });
#endif
  ...
  if (isHit) {
    ...
    DirectX::XMStoreFloat3(&hitResult->position, DirectX::XMVector3TransformCoord(
      DirectX::XMLoadFloat3(&hitResult->position), gameObject.lock()->transform.GetWorldTransform()));
    ...
    DirectX::XMStoreFloat3(&hitResult->normal, DirectX::XMVector3TransformNormal(
      Normal, gameObject.lock()->transform.GetWorldTransform()));
...
bool CompStage::IsInsideBox(const DirectX::XMFLOAT3& position, const DirectX::XMFLOAT3& positive, const DirectX::XMFLOAT3& negative)
{
  if (position.x < negative.x || position.x > positive.x)return false;
  if (position.y < negative.y || position.y > positive.y)return false;
  if (position.z < negative.z || position.z > positive.z)return false;

  return true;
}

bool CompStage::RayVsBox(const DirectX::XMFLOAT3& start, const DirectX::XMFLOAT3& end, const DirectX::XMFLOAT3& positive, const DirectX::XMFLOAT3& negative)
{
  float min[] = {
    negative.x,
    negative.y,
