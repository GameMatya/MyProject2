...
public:
  ...
  // 腕のノード番号
  void SetSplitID(const ANIM_AREA& bodyArea, const int& armJoint, const int& armTip);
...
private:
  ...
  std::array<DirectX::XMINT2, 2>  splitNodeId = { DirectX::XMINT2(-1,-1),  DirectX::XMINT2(-1,-1) }; // 腕の付け根と腕の先端のノード番号

  std::array<int, ANIM_AREA::MAX>       currentAnimeIndex = { -1,-1,-1,-1 };  // 再生中のアニメーション番号
