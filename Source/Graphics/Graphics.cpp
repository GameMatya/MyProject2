...
// Model
#include "Graphics/Shaders/FirstDeferredShader.h"
...
Graphics::Graphics(HWND hWnd) :hwnd(hwnd)
{
  ...
  // スプライトのシェーダー
  {
    spriteShaders[static_cast<int>(SHADER_ID_SPRITE::DEFAULT)] = std::make_unique<SpriteShader>("Shader/SpriteVS.cso", "Shader/SpritePS.cso");
    spriteShaders[static_cast<int>(SHADER_ID_SPRITE::BEND_SCREEN)] = std::make_unique<SpriteShader>("Shader/SpriteVS.cso", "Shader/BendScreenPS.cso");
  }
  // モデルのシェーダー
  {
    modelShaders[static_cast<int>(SHADER_ID_MODEL::FIRST_DEFERRED)] = std::make_unique<FirstDeferredShader>(device.Get());
  }
  // 静的モデルのシェーダー
  {
    staticModelShaders[static_cast<int>(SHADER_ID_MODEL::FIRST_DEFERRED)] = std::make_unique<StaticModelShader>("Shader/StaticBatchVS.cso", "Shader/FirstDeferredPS.cso");
  }
  // ポストプロセスのシェーダー
  {
    postShaders[static_cast<int>(SHADER_ID_POST::DEFAULT)] = std::make_unique<PostShader>("Shader/PostProcessDefaultVS.cso", "Shader/PostProcessDefaultPS.cso");
    postShaders[static_cast<int>(SHADER_ID_POST::SECOND_DEFERRED)] = std::make_unique<PostShader>("Shader/PostProcessDefaultVS.cso", "Shader/PBR_PS_Unreal.cso");
    postShaders[static_cast<int>(SHADER_ID_POST::SKY_BOX)] = std::make_unique<PostShader>("Shader/SkyboxVS.cso", "Shader/SkyboxPS.cso");
    postShaders[static_cast<int>(SHADER_ID_POST::COLOR_GRADING)] = std::make_unique<PostShader>("Shader/PostProcessDefaultVS.cso", "Shader/ColorGradingPS.cso");
    postShaders[static_cast<int>(SHADER_ID_POST::DISTANCE_FOG)] = std::make_unique<PostShader>("Shader/PostProcessDefaultVS.cso", "Shader/DistanceFogPS.cso");
  }
  ...
  debugRenderTarget = std::make_unique<RenderTarget>(screenWidth, screenHeight, DXGI_FORMAT_R8G8B8A8_UNORM);
