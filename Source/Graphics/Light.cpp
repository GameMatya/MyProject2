...
void Light::PushRenderContext(LightCB& cb)
{
  ...
  case	LightType::Directional:
  {
    ...
    cb.directionalLight.color = color * power;
    break;
  }
...
void Light::DrawDebugGUI()
{
  ...
  if (ImGui::TreeNode(lightTypeName[static_cast<int>(lightType)]))
  {
    ...
    case	LightType::Directional:
    {
      ...
      ImGui::TextColored({ color.x,color.y,color.z,1 }, (std::to_string(color.x) + std::to_string(color.y) + std::to_string(color.z)).c_str());
