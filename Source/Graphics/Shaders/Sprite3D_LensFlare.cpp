...
#include "Sprite3D_LensFlare.h"

Sprite3D_LensFlare::Sprite3D_LensFlare(ID3D11Device* device)
{
  // 頂点シェーダー
  {
    // ファイルを開く
    FILE* fp = nullptr;
    fopen_s(&fp, "Shader\\Sprite3D_LensFlare_VS.cso", "rb");
  ...
  // ピクセルシェーダー
  {
    // ファイルを開く
    FILE* fp = nullptr;
    fopen_s(&fp, "Shader\\Sprite3D_LensFlare_PS.cso", "rb");
    _ASSERT_EXPR_A(fp, "CSO File not found");
...
// 描画開始
void Sprite3D_LensFlare::Begin(ID3D11DeviceContext* dc, const RenderContext& rc)
{
  // Shader設定
  ...
  // 深度テクスチャ
  depthTexture = rc.DepthTexture;

}

// 描画
void Sprite3D_LensFlare::Draw(ID3D11DeviceContext* dc, const Sprite3D* sprite, const DirectX::XMFLOAT4& color)
{
...
// 描画終了
void Sprite3D_LensFlare::End(ID3D11DeviceContext* dc)
{
