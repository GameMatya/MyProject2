...
#include "Graphics/post_process/BrownFilter.h"

BrownFilterShader::BrownFilterShader(ID3D11Device* device)
{
  // 頂点シェーダー
  {
    // ファイルを開く
    FILE* fp = nullptr;
    fopen_s(&fp, "Shader\\BrownFilter_VS.cso", "rb");
  ...
  // ピクセルシェーダー
  {
    // ファイルを開く
    FILE* fp = nullptr;
    fopen_s(&fp, "Shader\\BrownFilter_PS.cso", "rb");
    _ASSERT_EXPR_A(fp, "CSO File not found");
...
// 描画開始
void BrownFilterShader::Begin(ID3D11DeviceContext* deviceContext, const RenderContext& rc)
{
  ...
  Constant cb;
  cb.pixel_detail = rc.brownFilterData.pixel_detail;
  cb.display_flat = rc.brownFilterData.display_flat;
  cb.noise_scroll = rc.brownFilterData.noise_scroll;
...
// 描画
void BrownFilterShader::Draw(ID3D11DeviceContext* deviceContext, const RenderContext& rc, const Sprite* sprite)
{
...
// 描画終了
void BrownFilterShader::End(ID3D11DeviceContext* deviceContext)
{
  ...
  deviceContext->PSSetShaderResources(0, ARRAYSIZE(srv), srv);
}
